...
const api = {
    post: (path, data) => Promise.resolve({
        data
    }),
    get: () => Promise.resolve({
        data: [1, 2, 3]
    }),
    put: (path, data) => Promise.resolve({
        data
    }),
...
describe('resource', () => {
    ...
    test('resourceUpdateRequest', async() => {
        ...
        dispatch(resourceUpdateRequest('resources', 4, {
            title: 'foo'
        }));
        await delay();
        expect(getList(getState(), 'resources')).toEqual([{
        ...
        dispatch(resourceUpdateRequest('resources', {
            title: 'foo'
        }, {
            foo: 'bar'
        }));
    ...
    test('resourceDeleteRequest', async() => {
        ...
        expect(getList(getState(), 'resources')).toEqual([1, 2, {
            foo: 'bar'
        }]);
        ...
        dispatch(resourceDeleteRequest('resources', {
            foo: 'bar'
        }));
        await delay();
        expect(getList(getState(), 'resources')).toEqual([2]);
