...
describe('RESOURCE_CREATE_SUCCESS', () => {
  it('adds the new data to the initial state', () => {
    expect(reducer(initialState, action(actions.RESOURCE_CREATE_SUCCESS, 1))).toEqual(
      state({ list: [1] }),
    );
  });
  ...
  it('prepends the new data to an existing state', () => {
    expect(
      reducer(state({ list: [1, 2] }), action(actions.RESOURCE_CREATE_SUCCESS, 3)),
...
describe('RESOURCE_LIST_READ_REQUEST', () => {
  it('keeps the list initial state', () => {
    expect(reducer(initialState, action(actions.RESOURCE_LIST_READ_REQUEST))).toEqual(
      state(),
    );
  });
  ...
  it('resets the list to initial state in an existing state', () => {
    expect(
      reducer(state({ list: [1, 2, 3] }), action(actions.RESOURCE_LIST_READ_REQUEST)),
    ).toEqual(state());
  });
});
...
describe('RESOURCE_LIST_READ_SUCCESS', () => {
  it('sets list in the initial state', () => {
    expect(
      reducer(initialState, action(actions.RESOURCE_LIST_READ_SUCCESS, [1, 2, 3])),
...
describe('RESOURCE_DETAIL_READ_REQUEST', () => {
  it('keeps the detail initial state', () => {
    expect(reducer(initialState, action(actions.RESOURCE_DETAIL_READ_REQUEST))).toEqual(
      state(),
    );
  });
  ...
  it('resets the detail to initial state in an existing state', () => {
    expect(
      reducer(state({ detail: 1 }), action(actions.RESOURCE_DETAIL_READ_REQUEST)),
    ).toEqual(state());
  });
});
...
describe('RESOURCE_DETAIL_READ_SUCCESS', () => {
  it('sets detail in the initial state', () => {
    expect(
      reducer(initialState, action(actions.RESOURCE_DETAIL_READ_SUCCESS, 1)),
    ).toEqual(state({ detail: 1 }));
  });

  it('overrides detail in an existing state', () => {
    expect(
      reducer(state({ detail: 1 }), action(actions.RESOURCE_DETAIL_READ_SUCCESS, 2)),
...
describe('RESOURCE_DELETE_SUCCESS', () => {
  it('keeps the initial state', () => {
    expect(reducer(initialState, action(action.RESOURCE_DELETE_SUCCESS))).toEqual(
      initialState,
    );
  });
